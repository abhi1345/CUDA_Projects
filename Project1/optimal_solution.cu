...
{
    __shared__ float Atemp[TILE_WIDTH][TILE_WIDTH];
    __shared__ float Btemp[TILE_WIDTH][TILE_WIDTH];
    ...
    int curRow = by * TILE_WIDTH + ty;
    int curCol = bx * TILE_WIDTH + tx;

    float tempSum = 0;

    for (int i = 0; i < matrixWidth/TILE_WIDTH; ++i) 
    {
        Atemp[ty][tx] = A[curRow * matrixWidth + (i * TILE_WIDTH + tx)];
        Btemp[ty][tx] = B[(i * TILE_WIDTH + ty) * matrixWidth + curCol];
        
        __syncthreads();

        for (int j = 0; j < TILE_WIDTH; ++j) 
        {
            tempSum += Atemp[ty][j] * Btemp[j][tx];
        }
        __syncthreads();
    }

    C[curRow * matrixWidth + curCol] = tempSum;
}
